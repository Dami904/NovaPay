      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);max-width:560px;">
        ...
        <tr>
          <td style="padding:0 40px 36px;text-align:center;">
            <a href="${explorerLink}" style="display:inline-block;background:#0f172a;color:#ffffff;font-size:14px;font-weight:600;padding:14px 28px;border-radius:8px;text-decoration:none;letter-spacing:0.02em;">
              View My Payment on Explorer &rarr;
            </a>
          </td>
        </tr>
...
app.post('/api/send-payment-emails', (req, res) => {
  if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) {
    return res.status(503).json({ error: 'Email service not configured — set GMAIL_USER and GMAIL_APP_PASSWORD in apps/api/.env' })
  }
...
app.listen(PORT, () => {
  console.log(`NovaPay API running on http://localhost:${PORT}`)
})
